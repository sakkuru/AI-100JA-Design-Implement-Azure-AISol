...
1. 歯車マークから、表示言語を変更できます

1. メイン ページで、「**+ New App(新規アプリ)**」ボタンをクリックします
...
1. 名前を**PictureBotLuisModel**と入力します

1. 文化を**日本語**にし、予測リソースを選択します

1. 「**完了**」をクリックします。  「効果的な LUIS アプリの作成方法」 ダイアログを閉じます。
...
1. 「**SearchPic**」という意図名を付け、「**完了**」をクリックします。
...
1. 「**予測リソース**」と「**キー**」が既に作成されていることがわかります。  **予測リソース**が表示されている場合は、「**アプリを公開する**」の次のセクションに進みます。
...
1. トップバーで「**発行**」をクリックします。

   > [!NOTE]
   > **実稼働** エンドポイントまたは **ステージング** エンドポイントに発行できます。「**Production**」 (実稼働) を選択し、2 つのエンドポイントの理由について確認してください。

1. 「**発行スロットと設定を選択する**」で、「**実稼働スロット**」を選択してから「**完了**」を選択します。
...
1. トップバーで「**テスト**」をクリックします。発話を入力して、意図が返されることを確認してください。これらのタスクの例を次に示します。
   ...
   | **トムに送信する** | 低いスコアが返されます | 再トレーニングが必要か、意図に一致しません |
    ...
    1. 「**上位のスコアの意図**」の横にある「新しい意図に割り当てます」をクリックし、ドロップダウンリストから「**SharePic**」を選択します。
    1. 「**テスト**」パネルを閉じます。
