...
1. Visual Studio で **PictureBot.sln** を開きます。
    ...
    > **注:** ラボ 3 を実行していない場合は、**Starter** ソリューションを使用できます。
...
1. 現在の `IStorage` 行を次の行に置き換えて、FileStorage ベースに変更します。
...
1. Azure Portal を開き、ストレージ アカウントに移動します。

1. **概要**ブレードから、**コンテナー**をクリックします。

1. **chatlog** コンテナが存在しない場合、**chatlog** コンテナを追加します。

1. まだ行っていない場合は、左側のメニューから「**アクセス キー**」をクリックして接続文字列を記録します
...
1. **emulator...** で始まるチャット ログ ファイルを選択し、「**編集**」 を選択します。

    ファイルには何が表示されていますか? 表示されると思っていたのに、表示されていないのは何ですか?
...
1. **PictureState.cs** の先頭に、以下のusingディレクティブを追加します。
    ```
    using System.Collections.Generic;
    ```

1. **Bots/PictureBot.cs** を開き、次のコードの**後**に
...
1. ボットと別の会話をします。ボットを停止し、最新の BLOB のログ ファイルを確認します。今度は何が表示されていますか?
    ...
    > 行き詰まってしまったときは? このラボのこの時点までのソリューションは、[Lab04-Log_Chat/code/Finished](./code/Finished/) にあります。`appsettings.json` ファイルに Azure Bot Service と Azure Storage の設定のキーを挿入する必要があります。このコードは、ソリューションとして実行するのではなく、参照として使用することをお勧めしますが、実行する場合は、必要なキーを必ず追加してください。
